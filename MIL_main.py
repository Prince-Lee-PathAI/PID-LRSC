...
from Visual_functions.vis_heatmap import interpret_bag
...
if __name__ == '__main__':
    # dataset_root =  'Cervix/New_Bags'
    dataset_root = 'Larynx/WSI_Multi_Bags'
    ########################## Hyparameters #########################
    ...
    paras.add_argument('--bags_len', type=int, default=1042)
    ...
    # paras.add_argument('--data_parallel', default=False,action='store_true')
    ...
    if args.run_mode == 'train':
        ...
        training_for_parallel(mil_feature=mil_feature, mil_head=mil_head, train_loader=train_loader,
                                val_loader=val_loader, test_loader=test_loader, epoch=args.epochs, gpu_device=args.gpu_device,
    ...
    if args.run_mode == 'test':
        ...
        if args.feat_extract:
            extracting_feat(mil_feature=mil_feature, mil_head=mil_head, test_loader=test_loader)
        elif args.bag_weight:
            interpret_bag(mil_feature=mil_feature, mil_head=mil_head, test_loader=test_loader)
        elif args.cluster_vis:
            Cluster_vis(mil_feature=mil_feature, mil_head=mil_head, test_loader=test_loader)
        else:
            testing_for_parallel(mil_feature=mil_feature, mil_head=mil_head,class_num=args.class_num,
